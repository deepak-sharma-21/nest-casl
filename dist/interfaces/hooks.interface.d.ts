...
export interface UserBeforeFilterHook<User extends AuthorizableUser<unknown, unknown> = AuthorizableUser, RequestUser = User> {
    run: (user: RequestUser) => Promise<User | undefined>;
}
export type UserBeforeFilterTuple<User extends AuthorizableUser<unknown, unknown> = AuthorizableUser, RequestUser = User> = [AnyClass, (service: InstanceType<AnyClass>, user: RequestUser) => Promise<User>];
