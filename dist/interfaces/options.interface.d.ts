...
export interface OptionsForRoot<Roles extends string = string, User extends AuthorizableUser<unknown, unknown> = AuthorizableUser<Roles>, Request = AuthorizableRequest<User>> {
    superuserRole?: Roles;
...
export interface OptionsForFeature<Roles extends string = string, Subjects extends Subject = Subject, Actions extends string = DefaultActions, User extends AuthorizableUser<unknown, unknown> = AuthorizableUser<Roles>> {
    permissions: AnyPermissions<Roles, Subjects, Actions, User>;
}
export interface OptionsForRootAsync<Roles extends string = string, User extends AuthorizableUser<unknown, unknown> = AuthorizableUser<Roles>, Request = AuthorizableRequest<User>> extends Pick<ModuleMetadata, 'imports'> {
